...
export default function ComparePanel({ documents, onCancel }: ComparePanelProps) {
  ...
  const handleRunComparison = async () => {
    if (selectedDocIds.length < 2) {
      setErrorMsg("Selecciona al menos dos documentos para realizar la comparación.");
      return;
    }
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(`Error en la comparación con AI: ${err.message || "No se pudo conectar con el servidor."}`);
    } finally {
      ...
      {comparisonResult ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center max-w-md mx-auto">
          ...
          <h3 className="text-base font-bold text-slate-100 mb-2">
            Generando Matriz Comparativa Cruzada
          </h3>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <span className="text-xs font-bold text-indigo-400 tracking-widest uppercase font-mono block">
              1. Selecciona los documentos (Mínimo 2)
            </span>
